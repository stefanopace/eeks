...
const { CommandFilter } = require('./filters/command');
...
class ConfigParser {
	...
	parseListConfig(nodeConfig, parentNode, matcher, setters) {
		...
		const children = nodeConfig.handlers.map(((resolver) => {
	...
	parseFilterConfig(nodeConfig){
		...
		return nodeConfig.filters.map((filterConfig) => {
			switch (filterConfig.type) {
				...
				case "command":
					return new CommandFilter(filterConfig.execute, filterConfig.placeholder, this.commandRunner, this.debug);
				default:
					return null;
			}
		})
